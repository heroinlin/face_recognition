...
class MSCeleb(data.Dataset):
    def __init__(self, image_root='data', annotation_folder=None, phase="train", cur_id=0):
        super(MSCeleb, self).__init__()
        self.image_root = image_root
        self.database_name = "MSCeleb"
        self.phase = phase
        self.cur_id = cur_id
        self.train_dir = os.path.join(self.image_root, "train")
        self.gallery_dir = os.path.join(self.image_root, "gallery")
        self.query_dir = os.path.join(self.image_root, "query")
        ...
        if self.phase == "train":
            self._prepare(self.train_dir, direction=0)
        elif self.phase == "test":
            self._prepare(self.gallery_dir, direction=0)
            self._prepare(self.query_dir, direction=1)
...
def test():
    ...
    train_dataset = MSCeleb(image_root)
